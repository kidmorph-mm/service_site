...
# Robust SMPL-X adult (pkl/npz) -> kidified POSED mesh exporter using best params
# - Supports .pkl and .npz inputs
# - Robustly reads pose from: global_orient/body_pose/... OR pose/full_pose/poses
# - Pose-preserving export: exports posed child mesh (OBJ/PLY)
# - Logs: canonical/posed heights, target height, child heights, scale_final
...
#
# adult_list.txt: one path per line (.pkl or .npz)
...
# height-adaptive template blending
...
class TemplateMorphParams:
    ...
    # arm length morph
    arm_len_scale: float = 0.96
    arm_sigma_out: float = 0.03
    arm_radius: float = 0.30
    arm_out_offset: float = 0.02
...
def load_any(path: str) -> Dict[str, Any]:
    ...
    raise RuntimeError(f"Unsupported input extension: {ext} ({path})")
...
def get_tensor_any(data: dict, keys: List[str], dim: int, default_zero: bool = True) -> torch.Tensor:
    for k in keys:
        if k in data and data[k] is not None:
            return fix_lastdim(to_torch(data[k]), dim)
    if not default_zero:
        raise KeyError(f"Missing keys (any of): {keys}")
    return torch.zeros((1, dim), device=DEVICE, dtype=torch.float32)


def _as_1d_np(x: Any) -> Optional[np.ndarray]:
    if x is None:
        return None
    a = np.asarray(x)
    if a.size == 0:
...
def parse_pose_bundle(data: dict) -> Dict[str, np.ndarray]:
    """
    Robustly parse pose from:
    - explicit keys: global_orient/body_pose/left_hand_pose/right_hand_pose/jaw_pose/leye_pose/reye_pose
    - or bundled: pose / poses / full_pose / full_poses

    SMPL-X common full_pose ordering (165):
      [global(3), body(63), jaw(3), leye(3), reye(3), lhand(45), rhand(45)]
    Some exporters store as:
      [global(3), body(63), lhand(45), rhand(45), jaw(3), leye(3), reye(3)]
    We try to pick the best by preferring an ordering where jaw/eyes are "smallish"
    and hands live in 45-d blocks. If explicit hand/jaw keys exist, we trust them.
    """
    out: Dict[str, np.ndarray] = {}

    # if explicit keys exist, use them (highest priority)
    if any(k in data for k in ["global_orient", "body_pose", "left_hand_pose", "right_hand_pose",
    ...
    def split_order_A(p: np.ndarray) -> Dict[str, np.ndarray]:
        # [g3, b63, jaw3, leye3, reye3, l45, r45]
        g = p[0:3]
        b = p[3:66]
        jaw = p[66:69]
        le = p[69:72]
        ...
        lh = p[75:120]
        rh = p[120:165]
        return {"global_orient": g, "body_pose": b, "jaw_pose": jaw, "leye_pose": le, "reye_pose": re,
                "left_hand_pose": lh, "right_hand_pose": rh}

    def split_order_B(p: np.ndarray) -> Dict[str, np.ndarray]:
        # [g3, b63, l45, r45, jaw3, leye3, reye3]
        g = p[0:3]
        b = p[3:66]
        lh = p[66:111]
        rh = p[111:156]
    ...
    if L >= 165:
        candA = split_order_A(pose[:165].copy())
        candB = split_order_B(pose[:165].copy())

        # heuristic: pick the one where jaw/eyes are not huge in magnitude
        def score_cand(c: Dict[str, np.ndarray]) -> float:
            jaw = c["jaw_pose"]
            le = c["leye_pose"]
            re = c["reye_pose"]
            return float(np.mean(np.abs(jaw)) + np.mean(np.abs(le)) + np.mean(np.abs(re)))

        out = candA if score_cand(candA) <= score_cand(candB) else candB
        return out

    # SMPL-style (72): [g3, b69] (no hands/face)
    if L >= 72:
        out["global_orient"] = pose[0:3].copy()
        out["body_pose"] = pose[3:66].copy()  # 63
        return out
...
# =========================
# Metrics: "어디가 어떻게 바뀌었는지"
# =========================
def metrics_from_joints_np(j_np: np.ndarray) -> Dict[str, float]:
    if max(J.values()) >= len(j_np):
        raise RuntimeError(f"Joint array too small: len={len(j_np)} max(J)={max(J.values())}.")
...
def deltas_dict(a: Dict[str, float], b: Dict[str, float]) -> Dict[str, float]:
    ...
    return {k: float(b.get(k, np.nan) - a.get(k, np.nan)) for k in keys}
...
def build_child_template(model, params: TemplateMorphParams) -> torch.Tensor:
    ...
    # (2.5) arm length morph (piecewise)
...
def load_best_params_txt(path: str) -> Tuple[TemplateMorphParams, Dict[str, float]]:
    ...
    tp = clamp_params(TemplateMorphParams(
        head_scale=float(hs),
...
def load_kid_heights(kid_audit_csv: str) -> np.ndarray:
    ...
    return kid_heights[(kid_heights >= lo) & (kid_heights <= hi)]
...
def convert_one(
    ...
    pose_bundle = parse_pose_bundle(data)

    num_betas = int(model_adult.num_betas)

    betas = get_tensor_any(data, ["betas", "beta", "shape", "shapes"], num_betas, default_zero=True)

    # Expression
    expression = get_tensor_any(data, ["expression", "expr", "exp"], 10, default_zero=True)

    # Transl (different datasets use different names)
    transl = get_tensor_any(data, ["transl", "trans", "translation", "cam_t", "t"], 3, default_zero=True)

    # Pose parts (prefer explicit parsing from bundle if present)
    global_orient = fix_lastdim(to_torch(pose_bundle.get("global_orient", np.zeros(3, np.float32))), 3)
    body_pose = fix_lastdim(to_torch(pose_bundle.get("body_pose", np.zeros(63, np.float32))), 63)
    left_hand_pose = fix_lastdim(to_torch(pose_bundle.get("left_hand_pose", np.zeros(45, np.float32))), 45)
    right_hand_pose = fix_lastdim(to_torch(pose_bundle.get("right_hand_pose", np.zeros(45, np.float32))), 45)
    jaw_pose = fix_lastdim(to_torch(pose_bundle.get("jaw_pose", np.zeros(3, np.float32))), 3)
    leye_pose = fix_lastdim(to_torch(pose_bundle.get("leye_pose", np.zeros(3, np.float32))), 3)
    reye_pose = fix_lastdim(to_torch(pose_bundle.get("reye_pose", np.zeros(3, np.float32))), 3)
    ...
    # If posed looks identical to canonical, warn (helps you detect "still T-pose")
    if abs(h_adult_pose - h_adult_can) < 1e-6 and float(torch.mean(torch.abs(body_pose)).item()) < 1e-6:
        print(f"[WARN] Pose seems all-zero -> likely T-pose input parsing issue: {in_path}")
    ...
    # keep your tuned deltas (txt small deltas) + fixed head/shoulder small behavior
    delta_small = {
        "head_scale": -0.02,
        "shoulder_scale": +0.08,
    ...
    # child model = copy adult weights
    ...
    # posed child vertices (THIS is what you want to export)
    ...
    meta = {
        ...
        "pose_stats": {
            "mean_abs_body_pose": float(torch.mean(torch.abs(body_pose)).item()),
            "mean_abs_global_orient": float(torch.mean(torch.abs(global_orient)).item()),
        },
...
def main():
    ...
    ap.add_argument("--in_path", default="")
    ...
    if args.in_path:
        in_paths = [args.in_path]
    elif args.in_list:
        in_paths = read_list(args.in_list)
    else:
        raise RuntimeError("Provide --in_path or --in_list")
    ...
    # infer num_betas from first sample
    s0 = load_any(in_paths[0])
    bet0 = None
    for k in ["betas", "beta", "shape", "shapes"]:
        if k in s0:
            bet0 = np.asarray(s0[k])
            break
    if bet0 is None:
        bet0 = np.zeros((10,), np.float32)
    bet0 = bet0.reshape(-1)
    num_betas = int(bet0.shape[0])
        ...
        for i, p in enumerate(in_paths, 1):
            try:
                ...
                row = {
                    ...
                    "pose_mean_abs_body_pose": meta["pose_stats"]["mean_abs_body_pose"],
